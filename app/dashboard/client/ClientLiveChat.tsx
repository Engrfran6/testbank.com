...
const ClientLiveChat = () => {
  ...
  const submitMessage = async () => {
    ...
    try {
      await messageFn({message: newMessage, sender: 'client'});
      setNewMessage('');
    } catch (error) {
  ...
  return (
    <div className=" p-4">
      <h1 className="text-lg font-bold">Client Live Chat</h1>
      <ul className="mb-4 bg-slate-100 p-4 rounded-lg shadow-md h-96 overflow-y-auto max-w-[24rem]">
        {messages.map((msg, index) => {
          ...
          return (
            <li
              key={index}
              className={`my-2 flex ${msg.sender === 'client' ? 'justify-end' : 'justify-start'}`}>
              <div
      ...
      <div className="flex space-x-2">
        ...
        />
        <button onClick={submitMessage} className="bg-blue-500 text-white p-2">
          Send
        </button>
      </div>
    </div>
